#!/bin/sh
#SBATCH --job-name=sample_and_split_all
#SBATCH --partition=owners
#SBATCH --array=1-85
#SBATCH --output=/scratch/users/briannac/logs/sample_and_split_all_%a.out
#SBATCH --error=/scratch/users/briannac/logs/sample_and_split_all_%a.err
#SBATCH --time=20:00:00
#SBATCH --mem=1GB
#SBATCH --mail-type=ALL
#SBATCH --mail-user=briannac@stanford.edu

### file at /home/groups/dpwall/briannac/y_chromosome_mismappings/src/covWAS/sample_and_split_all.sh
...
for I_TO_ADD in 0; do
    ...
    while read SAMPLE ; do
        ...
        \rm *all*
        txtfile=$SAMPLE.all.txt  
        ################### Properly paired pileups.  ###################
        echo "Counting all..."
        #if [ ! -f "$SAMPLE.all.txt.gz" ]; then
        
            # Collect coverages.
            /oak/stanford/groups/dpwall/computeEnvironments/samtools-1.10/bin/samtools depth  s3://ihart-hg38/cram/${SAMPLE/_LCL/-LCL}.final.cram -aa -G 0x400 -G 0x100 -G 0x200 | cut -f3 >  $txtfile
    ...
    \rm $SCRATCH/tmp/tmp_coverages_$SLURM_ARRAY_TASK_ID.txt
            ...
            for f in {2497..3217}; do 
                echo ${SAMPLE}.all.$f.txt
                sed  -i "1i $SAMPLE"  ${SAMPLE}.all.$f.txt
            done